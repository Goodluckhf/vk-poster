...
//    });
    ...
    this.loadPosts = function (group, count) {
        ...
        Request.api('wall.get', {domain: group, count: 100, v: 5.40}).done(function (data) {
            ...
            return Request.api('wall.get', {domain: group, count: 100, offset: 100, v: 5.40}).done(function (data) {
                var items = data.response.items;
                postByResponse(items);
            });
        }).then(function () {            
            return Request.api('wall.get', {domain: group, count: 100, offset: 200, v: 5.40}).done(function (data) {
                var items = data.response.items;
                postByResponse(items);
            });
        }).done(function() {
